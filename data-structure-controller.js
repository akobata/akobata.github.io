...
//        springStart.dayNumber;
	...
	function constructCalendarData(academicYear){
		...
		*/
		...
		ExecuteProgram(data.candidateEntryData);
		applyStrategies(data.candidateEntryData);
	...
	function SetHolidays(candidateEntry){
		...
		SetDayHoliday(candidateEntry, candidateEntry.monthMarkers["NOVEMBER"], "THU",4, "THANKSGIVING");
	...
	function applyStrategies(data){
		...
		var test = 0;
		while(errors.length > 0 && conflicts.length == 0){
			test++;
			if(test > 10)
				exit();
			console.log("HERE");
			...
			for(var i = 0; i < errors.length; i++){
				switch(errors[i]){
					case "ID MORE THAN 149":
						conflicts.concat(errors);
						break;
					case "AWD MORE THAN 180":
						console.log("awd shrinking");
						if(filter(data, data.boundaries["FALL_START"], data. boundaries["FALL_END"], isType, ["ACAD", "FINL", "COMM", "CONV", "INST"]).length >
							filter(data, data.boundaries["WINTER_START"], data. boundaries["WINTER_END"], isType, ["ACAD", "FINL", "COMM", "CONV", "INST"]).length){
							console.log("shrinking fall");
							errors = squeeze(data, "FALL");
							console.log(errors);
							if(errors.indexOf("AWD MORE THAN 180") >= 0){
								console.log("shrinking spring");
								errors = squeeze(data, "SPRING");
							}
						}
						else{
							console.log("shrinking spring");
							error = squeeze(data, "SPRING");
							if(errors.indexOf("AWD MORE THAN 180") >= 0){
								console.log("shrinking fall");
								errors = squeeze(data, "FALL");
							}
